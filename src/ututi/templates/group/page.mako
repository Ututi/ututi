  ...
  %else:
    <%self:rounded_block id="page-placeholder-frame">
      <div id="page-placeholder" class="content">
        ...
        <h2>${_('What is a group page and who sees it?')}</h2>
        <p>
          ${_('The group page is the face of the group. It may be public or visible only to group members, depending on configuration.')}
          ${_('You can change visibility settings in the <a href="%s">group settings</a> screen.') % c.group.url(action='edit') |n}
        </p>

        <h2>${_('What are the uses of a group page?')}</h2>
        ...
        <h2 style="padding-bottom: 0">${_('Edit your group page now!')}</h2>
      </div>
    </%self:rounded_block>
  %endif
</div>
...
<div style="padding-left: 10px">
  ${h.button_to(_('Edit page'), url(controller='group', action='edit_page', id=c.group.group_id))}
</div>
