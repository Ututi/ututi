...
<form method="post" action="${url(controller='profile', action='update')}"
  ...
  <div class="explanation-post-header">
    ...
    <p class="tip">
      %if c.user.profile_is_public:
      ${_("Uncheck the checkbox if you don't want your profile page to be visible to unregistered users and search engines.")}
      %else:
      ${_("Check the checkbox if you want your profile page to be visible to unregistered users and search engines.")}
      %endif
    </p>
    <p class="tip">
      <% user_url = c.user.url(qualified=True) %>
      ${_("You can also set a Ututi username to have a more personal URL of your profile page.")}
